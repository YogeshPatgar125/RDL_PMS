...
//     if (project.status !== 'Cancelled') acc.assigned += 1; // All except cancelled are assigned
//     return acc;
...
//         projectData.length - statuses.assigned
//       ],
...
//           {['Assigned Projects', 'Completed Projects'].map((title, index) => (
...
//                   <Typography 
//                     variant="h3" 
//                     color={index === 0 ? '#1e3a8a' : '#0d47a1'} 
//                     fontWeight="bold"
//                   >
//                     <b>{index === 0 ? statuses.assigned : statuses.completed}</b>
//                   </Typography>
//                 </CardContent>
//               </Card>
//             </Grid>
...
//               <Grid item xs={2}><Typography>Id</Typography></Grid>
//               <Grid item xs={4}><Typography>Project Name</Typography></Grid>
//               <Grid item xs={3}><Typography>Due Date</Typography></Grid>
//               <Grid item xs={3} xl={3}><Box sx={{ ml: 4 }}><Typography>Status</Typography></Box></Grid>
//             </Grid>
...
// import ChevronLeftIcon from '@mui/icons-material/ChevronLeft';
// import ChevronRightIcon from '@mui/icons-material/ChevronRight';
// import { fetchEmployeeProjects } from '../api/api'; // Make sure this path is correct
...
//         const teamLeaderProjects = await fetchEmployeeProjects(userId);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const userId = localStorage.getItem('userId'); // Make sure userId is stored
        if (!userId) throw new Error('User ID not found in localStorage');
        const role = localStorage.getItem('role'); // Make sure userId is stored
        if (!role) throw new Error('Role not found in localStorage');

        const employeeProjects = await fetchEmployeeProjects(userId, role);
        setProjectData(employeeProjects);
        setTotalProjects(employeeProjects.length);
        setLoading(false);
      } catch (error) {
        console.error('Error fetching team leader projects:', error);
        setLoading(false);
